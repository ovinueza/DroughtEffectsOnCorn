L.Control.SliderControl = L.Control.extend({
  ...
  onAdd: function(map) {
    ...
    $(sliderContainer).append(
      '<div id="leaflet-slider" style="width:200px"><div class="ui-slider-handle"></div><div id="slider-timestamp" style="width:200px; margin-top:10px;background-color:#FFFFFF"></div></div>'
    );
      ...
      slide: function(e, ui) {
        ...
        if (!!_options.markers[ui.value]) {
          ...
          if (_options.markers[ui.value].feature !== undefined) {
            if (_options.markers[ui.value].feature.properties.YEAR_MONTH) {
              if (_options.markers[ui.value])
                $("#slider-timestamp").html(
                  _options.markers[ui.value].feature.properties.YEAR_MONTH
                );
            } else {
            ...
            if (_options.markers[ui.value].options.time) {
              if (_options.markers[ui.value])
                $("#slider-timestamp").html(
                  _options.markers[ui.value].options.YEAR_MONTH.substr(0, 19)
                );
            } else {
              console.error("You have to have a time property");
            }
